...
static BOOL _DrawLine( _LPCDISPLAYDRV lpDispDrv, _LPRECTNODE lprn, _LINEDATA * lpLine, int x0, int y0, int x1, int y1 )
{
	...
	while( lprn )
	{
		...
		if( dx == 0 )
		{   // 垂直线 vert line
			if( x0 >= lprn->rect.left && x0 < lprn->rect.right )
			{
				int t, b;
				if( dy > 0 )
				{
		...
		{   // 水平线 hori line
			if( y0 >= lprn->rect.top && y0 < lprn->rect.bottom )
			{
				int l, r;
				if( dx > 0 )
				{
...
static int _ScanText( _LPFONT lpFont, LPCSTR lpstr, UINT uFormat, int* lpxWidth, int* lpwCount, int xClip, int nCount, int * lpEllipsisPos )
{
    ...
    *lpxWidth = *lpwCount = 0;
    ...
    while( *lpwCount < nCount )
    {
        ...
        {	//通常的字符
            cw = lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)lpstr, &bitmap );
            xWidth = bitmap.bmWidth;
	        firstPreFix = 0;
	    }
...
int WINAPI WinGdi_DrawText( HDC hdc, LPCTSTR lpstr, int nCount, LPRECT lpRect, UINT uFormat )
{
	...
	if( lpdc && lpdc->lpDispDrv )
    {
		...
		//
		//初始化块绘制结构
        textBitmap.bmPlanes = 1;
        textBitmap.bmBitsPixel = 1;
        textBitblt.lpDestImage = lpBitmap;
        textBitblt.lprcDest = &r;
        textBitblt.lpBrush = 0;
        textBitblt.lpSrcImage = &textBitmap;
        ...
        {	// 扫描字符串并得到一显示行 get new line			
            ...
            {    // 是否为tab字符 is tab char
				...
				{	//有省略号风格
					...
					if( bEllipsis[0] )
					{	//需要画省略号,得到字符的 字模
						bw = lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)bEllipsis, &textBitmap );
					}
					else 
						bw = lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)lpstr, &textBitmap );//得到字符的 字模
				...
				//得到字符显示的位置
                r.left = x; r.top = y; 
                r.right = x + textBitmap.bmWidth; 
                r.bottom = y + textBitmap.bmHeight;
                yUnderLine = r.bottom;
				//与裁剪区求交
                         ...
                         int * lpNextPos )
{
	...
	if( lpdc )
    {
        ...
        _LINEDATA lineData;
        RECT rectClip, r, rectSave, rMask;
        _FILLRGN aFillRgn;
        _BLKBITBLT textBitblt;
		_LPCDISPLAYDRV lpDispDrv = lpdc->lpDispDrv;
        ...
        while( *lpstr && count )
        {	//得到字模
            cw = lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)lpstr, &textBitmap );
			//得到字符的显示矩形
            r.left = x;
            r.top = y;
            r.right = x + textBitmap.bmWidth;
			...
			//准备下一个字符
	        if( lpNextPos != 0 )
	            x += textBitmap.bmWidth + *lpNextPos++;
	        else
	            x += textBitmap.bmWidth;
	        num++;
            if( count > (UINT)cw )
							  ...
							  int * lpNextPos )
{
	...
	{
		return _ExtTextOut( lpdc, x, y, option, lpRect, lpstr, count, lpNextPos );
	}
	return FALSE;
}
...
BOOL WINAPI WinGdi_TextOut( HDC hdc, int x, int y, LPCTSTR lpString, int count )
{
    ...
    {	//是否透明输出 ？
        if( lpdc->backMode == OPAQUE )
	        return _ExtTextOut( lpdc, x, y, ETO_OPAQUE, 0, lpString, count, 0 );
        else	//否
	        return _ExtTextOut( lpdc, x, y, 0, 0, lpString, count, 0 );
    }
    return FALSE;
}
							...
							)
{
		...
		( lpdc = _GetSafeDrawPtr( hdc ) ) != NULL )
    {
		...
		while( nCount > 0 )
		{
			...
			{	//输出
				if( lpdc->backMode == OPAQUE )
					return _ExtTextOut( lpdc, x, y, ETO_OPAQUE, 0, (LPCSTR)bString, nByte, 0 );
				else
					return _ExtTextOut( lpdc, x, y, 0, 0, (LPCSTR)bString, nByte, 0 );
								 ...
								 LPSIZE lpSize )
{
	...
	while( nCount > 0 )
	{
		lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)lpcstr, &bitmap );
    ...
    if( (lpdc = _GetHDCPtr( hdc )) != 0 )
    {
        ...
        while( nString > 0 )
        {
       		if( lpFit && lpSize->cx >= nMaxExtent )
				 break;
			//得到字符属性（宽和高）
			ws = lpFontDrv->lpWordBitmap( hFont, (LPCBYTE)lpszStr, &bitmap );
			//累加相关的值
            lpSize->cx += bitmap.bmWidth;
            v = bitmap.bmHeight;
            lpSize->cy = MAX( v, lpSize->cy );
...
static int DoExtSelectClipRgn( HDC hdc, HRGN hrgn, int mode )
{
    ...
    if( lpdc )
    {
		...
		{	//
			hrgnExpose = WinRgn_CreateRect( lpdc->deviceOrg.x, lpdc->deviceOrg.y, lpdc->deviceExt.cx, lpdc->deviceExt.cy );
		}
