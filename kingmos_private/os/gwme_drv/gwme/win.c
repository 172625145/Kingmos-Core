        ...
        if( lpwsChild )
        {
            ...
            {            
                    ...
                    WinRgn_RectInRegion(hrgnUpdate, &lpws->rectWindow) == TRUE )
                {
                    ...
                    retv = WinRgn_Combine( hrgnUpdate, hrgnUpdate, hrgn, RGN_DIFF );
                    WinGdi_DeleteObject( hrgn );
                    if( retv == NULLREGION )
                        break;
                }
...
BOOL WINAPI Win_InvalidateRect(HWND hWnd, LPCRECT lprc, BOOL bErase)
{
	...
	if( hWnd )
	{
		...
		if( lpws )
		{
			...
			if( IS_SHOWENABLE( lpws->dwMainStyle ) )  // 2003-08-05, 为 ShowDesktop修改
			{
...
static BOOL IsNeedChangeZOrder( _LPWINDATA lpwsPrev, _LPWINDATA lpws )
{
	...
	if( bPrev )
	{
		...
		while( lpwsTemp )
		{
			...
			//该窗口是被拥有的窗口吗？
			if( IS_SHOWENABLE( lpwsTemp->dwMainStyle ) )
			{	//是
                   ...
                   UINT uFlags )
{
	...
	if( (winpos.flags & SWP_NOACTIVATE) == 0 )
	{
...
static int __ExcludeChildrens( _LPWINDATA lpwsChild, HRGN hrgnExpose, LPRECT lprcClip )
{
    ...
    while( lpwsChild )
    {
		if( IS_SHOWENABLE( lpwsChild->dwMainStyle ) )  // 2003-08-05, 为 ShowDesktop修改
        {                                          
...
static int __ExcludeBrother( _LPWINDATA lpws, HRGN hrgnExpose, _LPWINDATA lpwsZUp )
{
    ...
    {
		if( IS_SHOWENABLE( lpwsBrother->dwMainStyle ) )  // 2003-08-05, 为 ShowDesktop修改
        {                                            
...
int GetExposeRgn( HWND hWnd, DWORD flags, HRGN * lphrgn )
{
    ...
    if( lpws == NULL || 
		IS_SHOWENABLE( lpws->dwMainStyle ) == 0  // 2003-08-05, 为 ShowDesktop修改
	  )
    {
