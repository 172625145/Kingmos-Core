...
static int _Symbol_WordMask( HANDLE, const BYTE FAR* lpText, _LPBITMAPDATA );
...
{
    lpMask->bmWidth = (int)handle;//16;
    lpMask->bmHeight = (int)handle;//16;
	if( (int)handle == 16 )
	{
		lpMask->bmWidthBytes = 2;
		lpMask->bmBits = (LPBYTE)&__bSymbolsFontMask16x16[( (*lpText) - '!' ) << 5];
	}
/*	else if( (int)handle == 24 )
	{
		lpMask->bmWidthBytes = 3;		
		lpMask->bmBits = (LPBYTE)&__bSymbolsFontMask24x24[( (*lpText) - '!' ) * 72];
	}
*/	
