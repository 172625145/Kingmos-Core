...
HandleAPICall:
     ...
     msr     cpsr_c, #SYSTEM_MODE | 0x80     @ switch to supervisor mode w/IRQs disable
     stmfd   sp!, {r0-r3}
	 mov     r1, sp
	 mov     r2, lr                @ return address

	 msr     cpsr_c, #ABORT_MODE | 0x80     @ switch to abort mode w/IRQs disable
     mov     r0, lr               @ r0 = api call info
	 mrs     r3, spsr             @ r3 = mode

     msr     cpsr_c, #SYSTEM_MODE    @ switch to supervisor mode w/IRQs enable
...
APICallRet:
	 ...
	 mov     r12, r0               @ r0 is return address

     ldr     r0, [sp, #0]          @ (r2) = return mode

	 add     sp, sp, #0x8          @ popup space for CALLRET struct

	 msr     spsr, r0              @ spsr = r2
	 ldmfd   sp!, {r0}             @ popup return value
	 movs     pc, r12
...
DirectRet:
    ...
    @ r0 = pc
	 ...
	 mov     r1,  sp
     msr     cpsr_c, #SYSTEM_MODE | 0x80    @ switch to system mode w/IRQs enable
     mov     sp, r1
     msr     cpsr_c, #SVC_MODE | 0x80    @ switch to system mode w/IRQs enable

	 msr     spsr, #SYSTEM_MODE              @ spsr = r2
     
	 movs    pc, r0
	 
@	 mov   r1, sp
@	 msr   cpsr_c, #SYSTEM_MODE | 0x80     @ switch to abort mode w/IRQs disable
@     mov   sp, r1
