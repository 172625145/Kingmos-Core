...
#define GET_FIRST_LEVEL_ENTRY( dwAdr ) ( (dwAdr) >> 20 ) //, dwVABase ) ( (dwAdr) >> 20 )
...
static THREAD_PAGE_TABLE pageTable;
static LPDWORD lpdwProcessPageEntry = 0;
...
{	//因为该线程结构是4k,最后1k作为 page table
	...
	*/
	if( pageTable.lpdwVirtualPageTable == NULL )
	{
		LPDWORD lpdwPageAdr = (LPDWORD)KHeap_Alloc( PAGE_SIZE );
		pageTable.lpdwVirtualPageTable = lpdwPageAdr;
		pageTable.lpdwPhyPageTable = (LPDWORD)( (DWORD)_GetPhysicalAdr( lpdwPageAdr ) | 1 ); // 1 mean second page
		pageTable.lpdwVirtualPageTable = (LPDWORD)CACHE_TO_UNCACHE(lpdwPageAdr);
		pageTable.lpdwFirstLevelEntry = FirstPageTableEntryAdr(0); 
		pageTable.lpdwSecondLevelEntry = pageTable.lpdwVirtualPageTable;//-1;		
		
		lpdwProcessPageEntry = FirstPageTableEntryAdr(0);
	}
} 
...
void ClearThreadPageTable( LPTHREAD_PAGE_TABLE lpCurPTS )
{
	...
	if( lpCurPTS )
	{
//		ASSERT( FirstPageTableEntryAdr(0) != lpCurPTS->lpdwFirstLevelEntry );
		*lpCurPTS->lpdwFirstLevelEntry = 0;
		//这句代码是重要的，它可以阻止在 LoadFailurePage 再修改其值（该值有可能是GetMMUContext设置的进程1M cace page ）
